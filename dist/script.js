...
let userMarker = null;
let userCircle = null;
const reachedWaypointIds = new Set();
function sendWaypointSignal(wp) {
...
function checkWaypointProximity(playerPosition) {
    waypoints.forEach((wp) => {
...
function checkQuestProximity(userLatLng) {
    const player = gameData.getPlayer();
    const quest = gameData.quests[player.currentQuestIndex];
    if (!quest)
        return;
...
function showQuestFoundPrompt(quest) {
